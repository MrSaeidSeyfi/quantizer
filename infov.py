...
ov_model_path = r"D:\projects\quantizer\model_vhf_pm11m_int8_openvino_model\_dsc23000_th6.2_bc0.368_int8_openvino_model"
ov_model = YOLO(ov_model_path, task="classify")

# Run inference
results = ov_model(image_path, imgsz=320, device="CPU")
top5_classes = [results[0].names[i] for i in results[0].probs.top5]
top5_confidences = [results[0].probs.data[i].item() for i in results[0].probs.top5]
